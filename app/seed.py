...
def seed_from_watchlist(conn: sqlite3.Connection, csv_path: Path) -> int:
    now_ms = _now_ms()
    count = 0
    with open(csv_path, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            conn.execute(
                ...
                VALUES (:symbol, :short_code, :name, :sector, 1, :now_ms, :now_ms)
...
async def seed_from_all_security_code(conn: sqlite3.Connection, client: CSEClient) -> tuple[int, int]:
    ...
    for rec in records:
        ...
        conn.execute(
            ...
            VALUES (:symbol, :short_code, :name, 0, :now_ms, :now_ms)
            ON CONFLICT(symbol) DO UPDATE SET
                name = excluded.name,
                updated_ms = excluded.updated_ms
            """,
